...
pub async fn run() {
    ...
    let mut state = State::get_instance().await.clone();
    // let state = Rc::get(&mut state).unwrap();
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        state.borrow_mut().load_data().await;
    }
    ...
    event_loop.run(move |event, control_flow| {
        match event {
            Event::WindowEvent {
                ref event,
                window_id,
            } if window_id == state.borrow_mut().window().id() => {
                if !state.borrow_mut().input(event) {
                    // UPDATED!
                    match event {
                        ...
                        WindowEvent::RedrawRequested => {
                            ...
                            state.borrow().window().request_redraw();
                            ...
                            match state.borrow_mut().render() {
                                Ok(_) => {}
                                // Reconfigure the surface if it's lost or outdated
                                Err(
                                    wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated,
                                ) => state.borrow_mut().resize(state.borrow().size),
